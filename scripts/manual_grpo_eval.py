...
def run_grpo_evaluation(task_info, reward_info, model_repo: str):
    ...
    try:
        original_model_path = snapshot_download(repo_id=original_model, cache_dir=cache_dir, ignore_patterns=None)
    ...
    try:
        model_path = snapshot_download(
            repo_id=model_repo, cache_dir=cache_dir, ignore_patterns=["*.h5", "*.ot", "*.msgpack", "*.pkl", "*.pth"]
        )
    ...
    docker_cmd = [
        ...
        "-e",
        f"DATASET_TYPE={dataset_type.model_dump_json()}",  # Use pydantic serialization
        "-e",
...
async def main():
    if len(sys.argv) != 3:
        ...
        print(
            "Example: python -m scripts.manual_grpo_eval ba444f8a-dd60-40e2-bdad-524e1e754457 microsoft/Phi-3-mini-128k-instruct"
        )
        sys.exit(1)
