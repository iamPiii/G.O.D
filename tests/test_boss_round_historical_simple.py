...
class TestHistoricalTaskSelection:
    ...
    async def test_get_random_historical_task_no_results(self, mock_psql_db):
        ...
        result = await get_random_historical_task_by_type(
            task_type="GrpoTask", start_date="2025-06-01", end_date="2025-08-01", min_successful_scores=10, psql_db=mock_psql_db
        )
        ...
        with patch("validator.tournament.boss_round_sync.get_task", return_value=sample_task):
            with patch("validator.tournament.boss_round_sync.add_task") as mock_add_task:
                ...
                result = await copy_tournament_task_into_general_miner_pool(
                    tournament_task_id=tournament_task_id, psql_db=mock_psql_db
                )
...
class TestBossRoundTaskCreation:
    ...
    async def test_create_historical_text_boss_round_tasks_success(self, mock_config):
        ...
        with patch("validator.tournament.task_creator.get_tournament_tasks", return_value=[]):
            ...
            with patch(
                "validator.tournament.task_creator.get_random_historical_task_by_type", side_effect=historical_ids
                ...
                with patch(
                    "validator.tournament.task_creator.copy_historical_task_into_boss_round_tournament", side_effect=copied_tasks
                ):
                    result = await _create_historical_text_boss_round_tasks(
                        tournament_id=tournament_id, round_id=round_id, config=mock_config
                    )
    ...
    async def test_create_historical_image_boss_round_tasks_success(self, mock_config):
        ...
        with patch("validator.tournament.task_creator.get_tournament_tasks", return_value=[]):
            ...
            with patch("validator.tournament.task_creator.get_random_historical_task_by_type", side_effect=historical_ids):
                copied_tasks = []
    ...
    async def test_create_historical_tasks_none_found(self, mock_config):
        ...
        with patch("validator.tournament.task_creator.get_tournament_tasks", return_value=[]):
            with patch(
                "validator.tournament.task_creator.get_random_historical_task_by_type", return_value=None
            ) as mock_get_historical:
...
class TestIntegrationFlow:
    ...
    async def test_complete_boss_round_historical_flow(self, mock_psql_db):
        ...
        historical_task_mock.model_copy = MagicMock(
            return_value=MagicMock(task_id=historical_task_id, task_type=TaskType.INSTRUCTTEXTTASK, status=TaskStatus.SUCCESS)
        )
    ...
    async def test_failed_tournament_task_sync_flow(self, mock_psql_db):
        ...
        failed_task.model_copy = MagicMock(return_value=MagicMock(task_id=failed_tournament_task_id, status=TaskStatus.FAILURE))
        ...
        with patch("validator.tournament.boss_round_sync.get_task", return_value=failed_task):
            with patch("validator.tournament.boss_round_sync.add_task"):
                ...
                general_task = await copy_tournament_task_into_general_miner_pool(
                    tournament_task_id=str(failed_tournament_task_id), psql_db=mock_psql_db
                )
...
class TestHistoricalTasksWithPartialExisting:
    ...
    async def test_create_text_boss_round_with_partial_existing(self, mock_config):
        ...
        existing_instruct_task = TournamentTask(
            tournament_id=tournament_id, round_id=round_id, task_id=uuid4(), pair_id=f"{round_id}_pair_001", group_id=None
        )

        with patch("validator.tournament.task_creator.get_tournament_tasks", return_value=[existing_instruct_task]):
            ...
            with patch(
                "validator.tournament.task_creator.get_random_historical_task_by_type",
                side_effect=[historical_dpo_id, historical_grpo_id],
            ) as mock_get_historical:
                ...
                with patch("validator.db.sql.tasks.get_task", return_value=existing_raw_task):
                    ...
                    with patch(
                        "validator.tournament.task_creator.copy_historical_task_into_boss_round_tournament", side_effect=new_tasks
                    ):
                        result = await _create_historical_text_boss_round_tasks(
                            tournament_id=tournament_id, round_id=round_id, config=mock_config
                        )
    ...
    async def test_create_image_boss_round_with_partial_existing(self, mock_config):
        ...
        existing_tasks = [
            TournamentTask(tournament_id=tournament_id, round_id=round_id, task_id=uuid4(), pair_id=pair_id, group_id=None),
            TournamentTask(tournament_id=tournament_id, round_id=round_id, task_id=uuid4(), pair_id=pair_id, group_id=None),
        ]
        ...
        with patch("validator.tournament.task_creator.get_tournament_tasks", return_value=existing_tasks):
            ...
            with patch(
                "validator.tournament.task_creator.get_random_historical_task_by_type", return_value=new_historical_id
            ) as mock_get_historical:
                ...
                with patch("validator.db.sql.tasks.get_task", side_effect=existing_raw_tasks):
                    ...
                    with patch(
                        "validator.tournament.task_creator.copy_historical_task_into_boss_round_tournament", return_value=new_task
                    ):
                        result = await _create_historical_image_boss_round_tasks(
                            tournament_id=tournament_id, round_id=round_id, config=mock_config
                        )
    ...
    async def test_real_boss_round_task_copy_flow(self, mock_psql_db):
        ...
        with patch("validator.tournament.boss_round_sync.get_task", side_effect=mock_get_task):
            with patch("validator.tournament.boss_round_sync.add_task", side_effect=lambda task, db: added_tasks.append(task)):
                with patch(
                    "validator.tournament.boss_round_sync.add_tournament_tasks",
    ...
    async def test_real_text_boss_round_creation_logic(self, mock_psql_db):
        ...
        with patch("validator.tournament.task_creator.get_tournament_tasks", return_value=[existing_task]):
            with patch("validator.db.sql.tasks.get_task", return_value=existing_raw):
                with patch(
                    "validator.tournament.task_creator.get_random_historical_task_by_type", side_effect=track_historical_fetch
                ):
                    with patch(
                        "validator.tournament.task_creator.copy_historical_task_into_boss_round_tournament",
                        side_effect=track_copy,
                    ):
                        # Call the REAL function
                        result = await _create_historical_text_boss_round_tasks(
                            tournament_id=tournament_id, round_id=round_id, config=config
                        )
