...
def create_test_dataset(output_path: Path) -> str:
    ...
    test_data = [
        ...
        {"instruction": "Translate 'Hello, how are you?' to Spanish.", "output": "Hola, ¿cómo estás?"},
        {
        ...
        {"instruction": "Calculate 15% of 200.", "output": "15% of 200 is 30."},
        {
...
def main():
    ...
    dataset_type = {"field_instruction": args.field_instruction, "field_output": args.field_output}
    ...
    docker_cmd = [
        ...
        "-v",
        f"{dataset_path.parent}:/workspace/input_data:ro",
        # Mount output directory
        "-v",
        f"{OUTPUT_DIR}:/output:rw",
        # Environment variables
        "-e",
        f"DATASET=/workspace/input_data/{dataset_path.name}",
        "-e",
        f"ORIGINAL_MODEL={MODEL_ID}",
        "-e",
        f"DATASET_TYPE={json.dumps(dataset_type)}",
        "-e",
        f"FILE_FORMAT=json",
        "-e",
        f"MODELS={MODEL_ID}",  # Testing the base model itself
        # Container and command
        args.container_image,
        "python",
        "-m",
        "validator.evaluation.eval_instruct_text",
    ]
