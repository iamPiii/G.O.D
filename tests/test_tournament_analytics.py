...
MOCK_GROUPS = [
    TournamentGroupData(group_id=f"{MOCK_TOURNAMENT_ID}_round_001_group_001", round_id=f"{MOCK_TOURNAMENT_ID}_round_001")
]
...
def mock_tournament_sql():
    with patch("validator.endpoints.tournament_analytics.tournament_sql") as mock:
        ...
        mock.get_tournament_group_members = AsyncMock(
            return_value=[type("MockMember", (), {"hotkey": p.hotkey})() for p in MOCK_PARTICIPANTS]
        )
...
async def test_tournament_with_group_and_final_rounds(
    mock_config, mock_tournament_sql, mock_task_sql, mock_tournament_scoring, mock_constants
):
...
if __name__ == "__main__":
    ...
    async def manual_test():
        ...
        with (
            patch("validator.endpoints.tournament_analytics.tournament_sql") as mock_sql,
            ...
            mock_sql.get_tournament_group_members = AsyncMock(
                return_value=[type("MockMember", (), {"hotkey": p.hotkey})() for p in MOCK_PARTICIPANTS]
            )
